...
public class AuthenticationConfig extends WebSecurityConfigurerAdapter {
    ...
    public void configure(AuthenticationManagerBuilder auth) throws Exception {
        auth
                ...
                .authoritiesByUsernameQuery("SELECT customer.username as username, role.role as role FROM customer INNER JOIN user_roles ON customer.id=user_roles.customer_id INNER JOIN role ON user_roles.role_id=role.id WHERE customer.username=?");            // setting the authorities table: 'user_roles'
    }
